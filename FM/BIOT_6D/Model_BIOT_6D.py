...
class BIOTEncoder(nn.Module):
    ...
    def forward(self, x, n_channel_offset=0, perturb=False, readout='pooling'):
        """
        x: [batch_size, channel, ts]
        output: [batch_size, emb_size] if readout=='pooling', else [batch_size, seq_len, emb_size]
        """
        ...
        token_emb = self.transformer(emb)
        if readout == 'pooling':
...
class BIOTClassifier(nn.Module):
    ...
    def forward(self, x, readout='pooling'):
        x = self.biot(x, readout=readout)
        x = self.classifier(x)
        return x
...
class UnsupervisedPretrain(nn.Module):
    ...
    def forward(self, x, n_channel_offset=0):
        emb = self.biot(x, n_channel_offset=n_channel_offset, perturb=True, readout='pooling')
        emb = self.prediction(emb)
        pred_emb = self.biot(x, n_channel_offset=n_channel_offset, perturb=False, readout='pooling')
        return emb, pred_emb
