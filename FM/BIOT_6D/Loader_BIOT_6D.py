...
class Loader_BIOT_6D(ModelLoader):
    """BIOT_6D loader: EEGData in, channel-mapped to 18-ch BIOT backbone + task head."""
    ...
    def __init__(self, eeg_dataset, **kwargs):
        ...
        pretrained_path = get_pretrained_models_path('EEG-six-datasets-18-channels.ckpt')
        if self.from_pretrain:
            model.biot.load_state_dict(torch.load(pretrained_path, weights_only=False))
            print(f"Loaded pretrained BIOT_6D from {pretrained_path}")
        else:
    ...
    def forward(self, x):
        ...
        Returns:
            Task head output (logits / regression).
        """
        x = self.chan_conv(x)
...
class EEGPreprocessor_BIOT_6D(Preprocessor):
    """Preprocessor for BIOT_6D: default target_fs=200, normalize_method='car'; see Preprocessor."""
    ...
    def preprocess(self, eeg_data, task_mode='Cross', train_percentage=0.3, **kwargs):
        """
        Full preprocessing pipeline for EEGData.

        Args:
            eeg_data: EEGData instance.
            task_mode: 'Cross' or 'Fewshot'.
            train_percentage: Few-shot train fraction (default 0.3).
            **kwargs: Passed to normalize_data.
