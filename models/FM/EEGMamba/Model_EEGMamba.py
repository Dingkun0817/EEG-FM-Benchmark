...
# os.environ['MAMBA_USE_TRITON'] = '0'  # disable Triton
# os.environ['MAMBA_USE_CUDA'] = '0'    # disable CUDA extension
import math
...
mamba_ssm.ops.triton.autotune = False
mamba_ssm.ops.triton.enabled = False
try:
...
def create_block(
    ...
    if attn_layer_idx is None:
        ...
        if hasattr(attn_layer_idx, 'default_factory'):
            attn_layer_idx = attn_layer_idx.default_factory()
        elif hasattr(attn_layer_idx, 'default'):
            attn_layer_idx = attn_layer_idx.default
        elif callable(attn_layer_idx):
...
class EEGMamba(nn.Module):
    ...
    def forward(self, x, mask=None):
        patch_size = 200
        bz, ch_num, t = x.shape
        seq_len = t // patch_size
        x = x.view(bz, ch_num, seq_len, patch_size)
