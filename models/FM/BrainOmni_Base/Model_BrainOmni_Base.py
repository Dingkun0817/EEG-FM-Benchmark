...
class BrainOmni(nn.Module):
    ...
    @torch.jit.ignore
    def load_frozen_tokenizer_ckpt(self, tokenizer_ckpt_path: Optional[str] = None):
        if tokenizer_ckpt_path is None:
            from utils.utils import get_pretrained_models_path
            tokenizer_ckpt_path = get_pretrained_models_path(
                'BrainOmni', 'braintokenizer', 'BrainTokenizer.pt'
            )
        if not os.path.exists(tokenizer_ckpt_path):
            raise FileNotFoundError(
                f"BrainTokenizer checkpoint not found: {tokenizer_ckpt_path}"
            )
